...
const Notification: FC<NotificationProps> = ({
  ...
  const loadingIcon = useMemo(
    () => (
      <div
        className={classNames(styles["icon-wrapper"], styles["icon-loading"])}
      >
        <img src={loadingSvg} />
      </div>
    ),
  ...
  const successIcon = useMemo(
    () => (
      <div className={styles["icon-wrapper"]}>
        <CheckCircleFilled className={classNames(styles["icon-success"])} />
      </div>
    ),
    []
  );
  ...
  const infoIcon = useMemo(
    () => (
      <div className={styles["icon-wrapper"]}>
        <ExclamationCircleFilled
          className={classNames(styles.icon, styles["icon-info"])}
        />
      </div>
    ),
    []
  ...
  const openNotification = (
    configItem: NotificationConfigItem,
    icon: ReactNode,
    duration: number
  ) => {
  ...
  useEffect(() => {
    if (open) handleNotification(type);
