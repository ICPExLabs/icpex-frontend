...
const options: (TradingFeeRateOption & { desc: string })[] = [
  {
    key: "Extremely Low",
    value: "0.01",
    desc: "Fees will be more market competitive.",
  },
  {
    key: "Low level",
    value: "0.3",
    desc: "Fees will be the same as most other pools.",
  },
  {
    key: "High level",
    value: "1",
...
const TradingFeeRateModal = forwardRef<
  TradingFeeRateModalRef,
  TradingFeeRateModalProps
>(({ onConfirm }, ref) => {
  ...
  const getCustomOption = (): TradingFeeRateOption => ({
    key: "Custom",
    value: customValue,
  });
  ...
  const handleConfirm = () => {
    ...
    onConfirm &&
      onConfirm(option.key === "Custom" ? getCustomOption() : option);
    handleCancel();
  };
  const Title = useMemo(() => {
    return (
      ...
      <div className={styles.body}>
        {options.map((item) => {
            ...
            return (
              <div
                ...
                className={classNames(
                  styles.custom,
                  option.key === key ? styles["custom-active"] : null,
                  showWarning ? styles["custom-warning"] : null
                )}
          ...
          return (
            <div
              ...
              className={classNames(
                styles.option,
                option.key === key ? styles["option-active"] : null
              )}
        ...
        {showWarning ? (
          <div className={styles.warning}>
            The trading fee rate must be between 0% to 10%
          </div>
        ) : null}
        <CommonButton
          type="primary"
