...
const TunableParametersModal: React.FC<{
  ...
  const [isSneed, setisSneed] = useState(false);
  const handleCancel = () => {
    onCancel?.();
  };
  ...
  const handleChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    const regex = /^\d*\.?\d{0,2}$/;
    if (regex.test(value)) setCustomValue(value);
  };
  ...
  const handleShowConfirm = (type: number, isSneed?: boolean) => {
  ...
  return (
    <>
      ...
      <Modal
        width={600}
        title={<div className={styles.title}>Set Tunable Parameters</div>}
        open={visible}
        centered
        maskClosable={false}
        footer={false}
        ...
        <div className={styles.body}>
          <div className={styles.icp}>
            ...
            <div className={styles.right}>
              <div className={styles.address}>
                {truncateString(data?.canisterId, 20)}
              </div>
              <span className={styles.space}>
                <Share
                  href={getPrincipalDashboardURL(data?.canisterId || "")}
                />
              </span>
              <span>
          ...
          <div className={styles.supply}>
            ...
            <span className={styles.num}>
              {formatBigNumberDisplay(data?.totalSupply)}
            </span>
          </div>
          ...
          {data?.canMint && (
            <div className={styles["add-tip"]}>
              {tipComponent(
                ...
                "The number of additional tokens that need to be issued this time."
              )}
            </div>
          )}
          {data?.canMint ? (
            <div className={styles.add}>
              <Input
                className={styles["input-token"]}
                value={customValue}
                placeholder="0-999,999,999,999"
                onChange={handleChange}
              />
              <Button
                className={styles["add-confirm"]}
                onClick={() => handleShowConfirm(ModalType.ADD)}
              >
                confirm
              </Button>
            </div>
          ...
          <div className={classNames(styles.add, styles.relinquish)}>
            <div
              className={classNames(styles["input-token"], styles["input-div"])}
            >
              {" "}
              {tipComponent(
                "Relinquish Ownership",
                "Relinquish token ownerships to make token more decentralized."
              )}
            </div>
            <Button
              className={styles["add-confirm"]}
              size="small"
              onClick={() => handleShowConfirm(ModalType.REMOVE, false)}
            >
              To BlackHole
            </Button>
            <Button
              className={styles["add-confirm"]}
              size="small"
              onClick={() => handleShowConfirm(ModalType.REMOVE, true)}
            >
              To Sneed Dao
            </Button>
          </div>
