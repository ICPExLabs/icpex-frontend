...
const MicroOperation: FC<MicroOperationProps> = ({
  isPrivate,
  isSingle,
  baseToken,
  quoteToken,
  ...
  const handleSequence = () => {
    if (isSingle) return;
    onSequence &&
      onSequence(sequence === "base-quote" ? "quote-base" : "base-quote");
  };
  ...
  useEffect(() => {
    if (!isSettingOpen) {
      if (Number(slippage) === 0) triggerSlippage("3");
    }
  }, [isSettingOpen]);
          ...
          <div className={styles.proportion}>
              ...
              {isSingle ? (
                <div className={styles.initPrice}>
                  ...
                  <Tooltip
                    ...
                    title="The minimum selling price set by this single-token pool."
                  >
                    <QuestionCircleFilled className={styles.info} />
                  </Tooltip>
                  : 1 {baseToken.symbol} = {i} {quoteToken.symbol}
                </div>
              ) : null}
            </div>
            <div className={styles["proportion-right"]}>
              ...
              <div className={styles.action} onClick={handleRealod}>
                <ReloadOutlined
                  className={classNames(loading ? styles.loading : null)}
                />
              </div>
              <div
                ...
                className={classNames(
                  styles.action,
                  isSettingOpen ? styles["action-active"] : null
                )}
