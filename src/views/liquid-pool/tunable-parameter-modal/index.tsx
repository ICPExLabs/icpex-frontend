...
const TunableParameterModal = forwardRef<
  TunableParameterModalRef,
  TunableParameterModalProps
>(({ onRefresh }, ref) => {
  ...
  const handlePriceChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    const regex = /^\d*\.?\d{0,2}$/;
    if (regex.test(value)) setPrice(value);
  };
  ...
  const handleConfirm = async () => {
    ...
    const fee = multiplyAndConvertToBigInt(
    ...
    const [err, _res] = await to(
      resetParameterOfPrivate(poolPrincipal, fee, i, k)
    );
    if (!err) {
  ...
  return (
    <>
      ...
      <Modal
        width={564}
        open={open}
        title={<CenterTitle title="Set Tunable Parameters" />}
        centered
        maskClosable={false}
        footer={false}
      ...
      >
        <div className="body">
          ...
          <ParameterItem
            ...
            tooltipTitle="Pools with lower transaction fees will attract more traders."
            value={`${tradingFeeRateOption.value}%`}
            onEdit={openTradingFeeRateModal}
          />
          <ParameterItem
