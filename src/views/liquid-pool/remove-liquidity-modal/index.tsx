...
const RemoveLiquidityModal = forwardRef<
  RemoveLiquidityModalRef,
...
>(({ onRefresh }, ref) => {
  ...
  const showModal: RemoveLiquidityModalRef["showModal"] = async (pool) => {
    setOpen(true);
  ...
  const openProgressModal = () => {
    ...
    const baseAmount =
      pool.type === "private" && selectedToken === "quote"
        ? 0n
        : multiplyAndConvertToBigInt(amount.base, baseToken.decimals);
    const quoteAmount =
      isSingle || (pool.type === "private" && selectedToken === "base")
        ? 0n
        : multiplyAndConvertToBigInt(amount.quote, quoteToken.decimals);
