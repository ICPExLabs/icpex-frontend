...
const RemoveProgressModal = forwardRef<
  RemoveProgressModalRef,
  RemoveProgressModalProps
>(({ onSuccess: afterSuccess }, ref) => {
  ...
  const handleRemove = async (args: any[]) => {
    ...
    const [err] = await to(
      removeLiquidity(p, poolCanister, baseAmount, quoteAmount, s, deadline)
    );
    if (!err) {
      handleSuccess();
    } else {
  ...
  const showModal: RemoveProgressModalRef["showModal"] = (
    pool,
    selectedToken,
    removeArgs
  ) => {
    ...
    if (pool.isSingle || pool.type === "private") {
      if (pool.type === "private") text += pool[selectedToken].symbol;
      else text += pool.base.symbol;
    } else {
      text += `${pool.base.symbol} and ${pool.quote.symbol}`;
    }
  ...
  return (
    <>
      ...
      <Modal
        width={517}
        title={<CenterTitle title="Remove LP in progress" />}
        open={open}
        centered
        maskClosable={false}
        footer={false}
