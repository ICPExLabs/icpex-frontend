...
const ImportTokenModal = forwardRef<ImportTokenModalRef, ImportTokenModalProps>(
  ({ onSuccess }, ref) => {
    ...
    const handleImport = async () => {
      ...
      if (res) {
        ...
        setError(
          `This canister id did not match the token standard "${protocol}"`
        );
      }
      setLoading(false);
    };
    ...
    return (
      <>
        <Modal
          width={574}
          title={<CenterTitle title="Import Token" />}
          open={open}
          centered
          maskClosable={false}
          footer={false}
            ...
            {buttonInfo.text === "Confirm" ? (
              <>
                <div className={styles.token}>
                  ...
                  <div>
                    <div className={styles.canisterId}>
                      {canisterId}{" "}
                      <Share href={getPrincipalDashboardURL(canisterId)} />
                    </div>
                    <div className={styles.view} onClick={() => goToken()}>
                      View token info
                    </div>
                  </div>
                </div>
                <Alert
                  description="Anyone can create a token with any name and LOGO on Internet Computer. If you purchase fake token, it may result in loss of assets. Please carefully identify the authenticity of tokens before investing!"
                  type="warning"
                  showIcon
                  icon={
                ...
                <div className={styles.agree}>
                  <Checkbox checked={agree} onChange={handleAgree} /> I have
                  read the risk warning carefully and agree to do so at my own
                  risk.
                </div>
              </>
            ) : null}
