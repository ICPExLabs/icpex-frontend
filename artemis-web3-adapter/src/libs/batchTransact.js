export const BatchTransaction = class BatchTransaction {
    ...
    _prepareTrxArry = function () {
        ...
        self.trxArray.forEach((subArray, i) => {
            subArray.forEach((el, j) => {
                ...
                const success_function = self.trxArray[i][j].onSuccess;
                const failed_function = self.trxArray[i][j].onFail;
    ...
    async _processBatch() {
        ...
        if (['plug'].includes(this._adapterObj.walletActive)) {
        ...
        } else if (['plug','stoic', 'dfinity', 'astrox', 'nfid','metamask','bitfinity'].includes(this._adapterObj.walletActive)) {
            try {
                for (const trxStepItem of self.trxArray) {
                    if (self.state == 'error' || self.state == 'done') break;
                    if (trxStepItem.length){
